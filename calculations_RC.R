...
# The data will be aggregated by 1 
...
if(Sys.info()[7] == "rose775"){   
  ...
  wd <- "/Volumes/cchpc/"
  # Need output wd location in RCS (e.g.,"R:/" for Kevin)
  # wd_out <- 
...
## Load data
site_IDs <- unique(substr(list.files(path = paste0(wd, "clean/1_sec/")), 1, 6))
...
for (i in site_IDs){
  ...
  df <- list.files(path = paste0(wd, "clean/1_sec/"), pattern = i, full.names = T) %>%
    map_df(~read_csv(.))
...
## Heating and cooling related calculations:
  ...
  heat_output_btu_h = ifelse(is.na(operating_mode), NA,
                             ifelse(operating_mode %in% c("Heating-Aux Only",
                                                          "Heating-HP Only",
                                                          "Heating-Aux/HP",
                                                          "Defrost"),
      ...
      (SA_temp_F - RA_temp_F), 0)),                                 # Temperature delta
  

  # Cooling output
    # Q-cooling = (dry air density) * (blower airflow rate) * (specific heat) * (delta Temp) / (1 + Humidity Ratio)
  cooling_output_btu_h = ifelse(is.na(operating_mode), NA,
                                ifelse(operating_mode == "Cooling",
      0.0765 *                                                          # Density of air at 15C (lb/ft3)
      supply_flow_rate_CFM * 60 *                                       # CFM * min/hour
      (0.24 + 0.444 *  supply_humidity_ratio) *                         # Specific heat capacity (Btu/F-lb)
      (SA_temp_F - RA_temp_F) /
      (1 + supply_humidity_ratio), 0)),

  # Create a column for the power only in cooling mode or heating mode to calculate COP
  HP_system_pwr_heating_kW = ifelse(operating_mode == "Cooling", HP_system_pwr_kW, 0),
  HP_system_pwr_cooling_kW = ifelse(operating_mode %in% c("Heating-Aux Only",
                                                          "Heating-HP Only",
                                                          "Heating-Aux/HP",
                                                          "Defrost"), 
                                    HP_system_pwr_kW, 0)
...
## Write to file calculated 1-second data
write_csv(df, paste0(wd_out, "calculated_data/1_sec/", i, ".csv"))



## Aggregate data at 1-hour, 1-minute, and 5-minute intervals and then write to file
  # Drop categorical variables that do not make sense to aggregate:
df <- df %>% select(-number_aux_legs,
                    -HP_cycle_runtimes,
                    -defrost_cycle_runtimes,
                    -HP_status,
                    -operational_issue,
                    -supply_flow_rate_CFM,
                    -partial_water_pressure_supply,
                    -supply_humidity_ratio,
                    -reversing_valve_signal_V)
...
write_csv(df_1m, paste0(wd_out, "calculated_data/1_min/", i, ".csv"))
